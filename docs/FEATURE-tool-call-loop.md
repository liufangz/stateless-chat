...
## Addendum — bash tool: sandbox removed in favor of direct host execution

The `bash` tool (`packages/worker/src/tools/bash.ts`) no longer spawns a docker container. It now runs the model's shell commands directly on the host via `sudo -n -u opc -- env ... bash -s`, dropping privileges to uid 1000 (`opc`, a non-sudo system user) the same way the docker sandbox previously ran commands as `1000:1000` inside the container. The repo at `/home/ubuntu/stateless-chat` is writable in place — there is no more read-only `/repo` + scratch `/work` split, so the model builds and edits directly in the checkout. This mirrors how the pi coding agent runs its own bash tool as its own (non-root) user rather than sandboxing via containers.
